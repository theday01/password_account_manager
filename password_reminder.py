...
class PasswordReminder:
    ...
    def _check_accounts(self):
        try:
            ...
            self.logger.info(f"Found {len(accounts_to_remind)} accounts older than 1 month")
    ...
    def start(self):
        self.logger.info("Starting PasswordReminder timer (first check in 10 minutes)")
        # Delay the first check to avoid slowing down the UI at startup
        self.timer = threading.Timer(600, self._check_accounts)
        self.timer.daemon = True
        self.timer.start()
